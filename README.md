...
static/src/javascript/network-diagram-init.js  ウィジェットの初期化コード（あえて外部ファイルにしています。理由は下記）
...
1. **ファイルをコピー**
   ...
   - `static/src/javascript/network-diagram-init.js` → 既存の `static/src/javascript/` 配下
...
## トラブルシューティング: チャートが表示されなくなった場合

以前 `_network_diagram.html` に直接 `<script>...`(inline)でウィジェットの初期化コードを書いていましたが、これが原因でチャートが消える不具合がありました。

**原因**: inline `<script>` の中で `//` 形式の行コメントを使っていたため、何らかの理由（HTMLミニファイア、テンプレートの空白圧縮など）でそのブロックが改行なしの1行に潰されると、最初の `//` 以降 — つまり実際のコードも含めて — 全部コメントアウトされてしまい、`NetworkDiagram.init(...)` が一切呼ばれなくなっていました（`Unexpected end of input` というJSエラーで確認済みです）。

**対策**: 初期化コードを `static/src/javascript/network-diagram-init.js` という外部ファイルに完全に切り出しました。外部`.js`ファイルはHTMLテンプレートのような改行圧縮の対象にならないため、この問題は起きません。`_network_diagram.html` 側はこのファイルを `<script src="...">` で読み込むだけになっています。

もし今後同様に「チャートだけ消える」不具合が起きたら、まずブラウザの開発者ツール(F12)のConsoleタブでJSエラーが出ていないか確認してみてください。
